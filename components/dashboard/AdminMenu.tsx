...
const findParentIds = (items: MenuItem[], targetId: string, path: string[] = []): string[] | null => {
  ...
  useEffect(() => {
    if (isActive && !isOpen) {
      ...
      if (parents.length > 0 || isActive) {
        const newOpenMenus = [...new Set([...openMenus, ...parents, item.id])];
        setOpenMenus(newOpenMenus);
        localStorage.setItem('openMenus', JSON.stringify(newOpenMenus));
      }
    }
    ...
    <div className="relative">
      {item.path ? (
        ...
        >
          ...
          <span className="text-sm whitespace-nowrap">{item.title}</span>
          <ChevronDown
            className={cn('ml-auto w-4 h-4 text-slate-400 transition-transform duration-200', {
              'rotate-180': isOpen
            })}
          />
          {isActive && (
  ...
  useEffect(() => {
    ...
    if (activePath.length > 0) {
      setOpenMenus((prevOpenMenus) => {
        const newOpenMenus = [...new Set([...prevOpenMenus, ...activePath])];
        localStorage.setItem('openMenus', JSON.stringify(newOpenMenus));
        return newOpenMenus;
      });
    }
